...
func GetDailyTokenUsageByUserId(userId int, startTime int64, endTime int64, location *time.Location) ([]DailyTokenUsage, error) {
	...
	err := DB.Table("quota_data").
		Select("created_at, sum(token_used) as token_used").
		Where("user_id = ? and created_at >= ? and created_at <= ?", userId, startTime, endTime).
		Group("created_at").
		Order("created_at ASC").
		Scan(&hourlyUsage).Error
	if err != nil {
		return nil, err
	}

	dailyUsage := make([]DailyTokenUsage, 0, len(hourlyUsage))
	for _, hourly := range hourlyUsage {
		date := time.Unix(hourly.CreatedAt, 0).In(location).Format("2006-01-02")
		lastIndex := len(dailyUsage) - 1
		if lastIndex >= 0 && dailyUsage[lastIndex].Date == date {
			dailyUsage[lastIndex].TokenUsed += hourly.TokenUsed
			continue
		}
		dailyUsage = append(dailyUsage, DailyTokenUsage{
			Date:      date,
			TokenUsed: hourly.TokenUsed,
		})
	}
